...
int main(int argc, char* argv[])
{
	...
	// If build mode is on, then we just need to compile and save the binary, no need to run anything.
	...
	const std::vector<byte> buffer(bin_size);
	bin_file.read((char*)buffer.data(), bin_size);

	// Run the virtual machine with the binary's byte code.
	vm virtual_machine(buffer);
